...
sbuild -c resolute-amd64 --dist=resolute --no-run-lintian --extra-repository="deb [trusted=yes] https://ppa.launchpadcontent.net/kobuk-team/oneapi/ubuntu resolute main" --build-path=""
```
...
```bash
sudo apt install autopkgtest dpkg-dev cmake libstb-dev ocl-icd-opencl-dev dpclang-6 pkg-config
```
